...
class TestSshTargetParse:
    ...
    def test_missing_colon_raises(self):
        with pytest.raises(ValueError, match='Invalid SSH target'):
            SshTarget.parse('alice@server.example.com')

    def test_ssh_destination_property(self):
        t = SshTarget(user='alice', host='server', zarr_root='/data')
        assert t.ssh_destination == 'alice@server'

    def test_empty_zarr_root(self):
        t = SshTarget.parse('alice@host:')
        assert t.zarr_root == ''

    def test_rsplit_uses_last_colon(self):
        """rsplit(':', 1) means the last colon is the delimiter."""
        t = SshTarget.parse('alice@host:/data/zarr:v2')
        # host_part = 'alice@host:/data/zarr', zarr_root = 'v2'
        assert t.zarr_root == 'v2'
...
class TestSshRunnerCommandBuilding:
    def test_default_port_no_p_flag(self):
        target = SshTarget(user='a', host='h', zarr_root='/z')
    ...
    def test_custom_port_includes_p_flag(self):
        target = SshTarget(user='a', host='h', zarr_root='/z', port=2222)
        runner = SshRunner(target=target)
        args = runner._ssh_args()
        idx = args.index('-p')
...
class TestSshRunnerRun:
    ...
    def test_successful_json_response(self):
        ...
        with patch('subprocess.run', return_value=result):
            data = self._runner().run('list-stores', '/z')
        assert data['data'] == 42
